...
本パッケージ（`Move_LookAtSmooth_002`）は `Move_LookAtSmooth_001`（無料版）の有料上位版です。
乗り物・ドローンなどZ-up以外の向きに傾く対象に追従できるUpAxis機能を搭載しています
（詳細は上記パラメータ一覧の`UpAxis`を参照）。
...
本パッケージは無料版（`Move_LookAtSmooth_001`）と同じクラス名・namespaceを使用しています。
**すでに`Move_LookAtSmooth_001`をプロジェクトに導入している場合は、`Move_LookAtSmooth_002`を
インポートする前に必ず001をプロジェクトから削除してください**。両方を同時に導入すると、クラス名の
重複によりコンパイルエラーになります。

---

もしこのアセットが役に立ちましたら、ぜひレビューを書いて応援していただけると開発の励みになります！
→ [レビューはこちら](https://www.fab.com/listings/9f156df3-0e0a-4faf-a1c1-57476c214b86)
